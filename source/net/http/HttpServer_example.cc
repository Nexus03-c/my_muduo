...
void RequestCallback(const HttpRequest& request, HttpResponse& response) {
    LOG_INFO<<"Method: "<<request.MethodToString()<<" "<<request.path();
    if(request.path() == "/") {
        if(request.version() == HttpRequest::kHttp10) {
            response.set_version("HTTP/1.0");
        } else if(request.version() == HttpRequest::kHttp11) {
            response.set_version("HTTP/1.1");
        }
        response.set_status(HttpResponse::kOk);
        response.set_status_msg("OK");
        response.SetContentType("text/plain");
        response.add_header("Server", "MyMuduo");
        //response.set_body("hello, world!\n");
        response.set_body("I love ZYF baby!\n");
        ...
        response.set_status(HttpResponse::kNotFound);
...
int main() {
    ...
    EventLoop loop;
    HttpServer server(&loop, InetAddress("0.0.0.0", 8000), "HttpServer");
    server.setHttpCallback(RequestCallback);
